...
public class Calculos {
	...
	public void calcularconsumo(Residencia r, int i) {
		...
		for (int x=0;x<r.getComodo().size();x++) {
			for(int y=0;y<r.getComodo().get(x).getUtensilios().size();y++) {
				...
				teste = "" +r.getComodo().get(x).getUtensilios().get(y).getConsumotot();
			}
		}
	}
	...
	public void relatorioConsumo(Residencia r) {
		...
		/*for (int x=0;x<r.getComodo().size();x++) {
			FrameHome.relconsumo.append(r.getComodo().get(x).toString() + "\n");
			somacomodo = new double[r.getComodo().size()];
			for(int y=0;y<r.getComodo().get(x).getUtensilios().size();y++) {
				FrameHome.relconsumo.insert(y, r.getComodo().get(x).getUtensilios().get(y).toString() + "\n");
			}
		FrameHome.relconsumo.append( somacomodo[x]);
		}
		FrameHome.relconsumo.append( "\n" + getSomaconsumo());*/
		for (int x=0;x>r.getComodo().size();x++) {
			JOptionPane.showMessageDialog(null,r.getComodo().get(x).toString());
		}
	}
	...
	//public String relatorio1(Residencia r) = "relatorio quarto" + r.getComodo().get(x).toString();
}
